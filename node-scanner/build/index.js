...
// config.json
var machine_id = "compose_machine_id_1";
var path_config = [
  {
    file_path: "/usr/src/node-scanner",
    files: [
...
// scanner.ts
var base_path_array = [];
path_config.forEach((val) => {
  base_path_array.push(import_path.default.format({
    dir: val.file_path
  }));
});
var file_path_array = [];
path_config.forEach((config) => {
  base_path_array.forEach((base_url) => {
    config.files.forEach((val) => {
      file_path_array.push({ base_url: import_path.default.join(base_url, val), file_name: val });
    });
  });
});
var machine_id2 = machine_id;
async function composeReader(file_path, file_name) {
  try {
    let service_list = [];
    let doc = js_yaml_default.load(import_fs.default.readFileSync(file_path, "utf-8"));
    ...
    const result = await axios_default.post(`${process.env.MACHINE_API_URL}`, {
...
async function composeCompare(hash_store2) {
  try {
    file_path_array.forEach(async (val) => {
      let fileBuffer = import_fs.default.readFileSync(val.base_url);
      const hex = import_crypto.default.createHmac("sha256", machine_id).update(fileBuffer).digest("hex");
      const objIndex = hash_store2.file.findIndex(
        (obj) => obj.file_name === val.file_name
      );
      if (hash_store2.file[objIndex]?.hash !== hex) {
        await composeReader(val.base_url, val.file_name);
      }
      if (objIndex === -1) {
        hash_store2.file.push({ file_name: val.file_name, hash: hex });
      } else {
        hash_store2.file[objIndex].hash = hex;
      }
    });
  } catch (error) {
    console.log(error);
...
async function composeReaderCallBack(composeCompare2, machine_id3) {
  try {
    console.log("Entered Callback");
    const result = await composeCompare2(hash_store);
  } catch (error) {
    console.log(error);
  }
}
